...
open_content() {
    ...
    if [[ "$target" == *.excalidraw.md ]] || [[ "$target" == *.base ]] || [[ "$mode" == "OBSIDIAN" ]]; then
        ...
        # Priorizamos bob ya que queremos fijar la version por tema de bugs y demas
        ...
        local inner_cmd="cd '$NOTE_PATH' && '$NVIM_BIN' '$target'; python3 '$HOME/.config/rofi/bin/dependencies/image_compresion.py'; python3 '$HOME/.config/rofi/bin/dependencies/update-todos.py'; if [ -n \"\$(git -C '$NOTE_PATH' status --porcelain)\" ]; then bash '$SYNC_SCRIPT' '$NOTE_PATH'; else notify-send 'Obsidian' 'Sin cambios, saltando sync.'; fi"
        ...
        if [[ "$target" == "$NOTE_PATH"/* ]]; then
            ...
            # setsid kitty --title="notes" -e tmux new-session -A -s "$session_name" bash -c "'$NVIM_BIN' '$target'" >/dev/null 2>&1 </dev/null &
            setsid kitty --title="notes" -e bash -c "'$NVIM_BIN' '$target'" >/dev/null 2>&1 </dev/null &
        fi
    fi
}
