...
update_todos() {
  local script_path="$HOME/bin/pickers/dependencies/update-todos.py"
...
parse_selected() {
  local SELECTED=$1
  local script_path="$HOME/bin/pickers/dependencies/note-formatter.py"
  if [ -f "$script_path" ]; then
...
optimize_attachments() {
  local attachment_script="$HOME/bin/pickers/dependencies/image_compresion.py"
...
edit_and_sync() {
  ...
  nvim "$target"
  python3 "$HOME/bin/pickers/dependencies/image_compresion.py"
  python3 "$HOME/bin/pickers/dependencies/update-todos.py"
...
open_content() {
  ...
  if [[ "$target" == *.excalidraw.md ]] || [[ "$target" == *.base ]] || [[ "$mode" == "OBSIDIAN" ]]; then
    ...
    local inner_cmd="cd '$NOTE_PATH' && nvim '$target'; python3 '$HOME/bin/pickers/dependencies/image_compresion.py'; python3 '$HOME/bin/pickers/dependencies/update-todos.py'; if [ -n \"\$(git -C '$NOTE_PATH' status --porcelain)\" ]; then bash '$SYNC_SCRIPT' '$NOTE_PATH'; else notify-send 'Obsidian' 'Sin cambios, saltando sync.'; fi"

    # Usar nvim solo o solo tmux
    ...
    if [[ "$target" == "$NOTE_PATH"/* ]]; then
      ...
      setsid kitty --title="notes" -e bash -c "$inner_cmd" >/dev/null 2>&1 </dev/null &
    else
      # setsid kitty --title="notes" -e tmux new-session -A -s "$session_name" bash -c "nvim '$target'" >/dev/null 2>&1 </dev/null &
      setsid kitty --title="notes" -e bash -c "nvim '$target'" >/dev/null 2>&1 </dev/null &
    fi
  fi
}
...
sync_notes() {
  ...
  if curl -s --head --request GET 1.1.1.1 --connect-timeout 2 &>/dev/null; then
    ...
    " PC Priority")
      ...
      # Limpieza de bloqueos previos y asegurar rama
    ...
    " Standart")
      ...
      optimizar_vault
      git add -A
      # Comprobamos si de verdad hay cambios antes de comitear al aire
      if ! git diff --cached --quiet; then
        git commit -m "Sync PC: $(date +%R)"
      fi
      ...
      optimizar_vault
      git add -A
      # Aquí también usamos --cached para detectar cualquier cosa que hayan hecho los scripts
      if ! git diff --cached --quiet; then
        git commit -m "Auto-compresión post-descarga"
      fi
    ...
    "󰊄 Mobile Priority")
      ...
      # Abortamos cualquier conflicto que se haya quedado colgado localmente
      git rebase --abort >/dev/null 2>&1 || true
      git merge --abort >/dev/null 2>&1 || true
      git checkout main >/dev/null 2>&1 || true
      ...
      git clean -fd # AHORA SÍ: aniquilamos los archivos locales no rastreados

      optimizar_vault
      git add -A
      if ! git diff --cached --quiet; then
        git commit -m "Limpieza post-descarga móvil"
        git push origin main
      fi
      ;;
...
"$DAILY_NOTE")
  ...
  ;;
"$NEW_NOTE")
  # Obtener el directorio de quicknotes
  dir_full=$(get_folder_by_pattern "$QUICKNOTES_PATTERN")
  mkdir -p "$dir_full"

  # Generar nombre automático con fecha y hora
  filename="note_$(date +%Y%m%d_%H%M%S).md"
  final_path="$dir_full/$filename"

  # Crear y abrir la nota inmediatamente
  touch "$final_path"
